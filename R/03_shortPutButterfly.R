...
#'@description
#'This strategy consists of a short position in an OTM (out of the money) put option with a strike price $X1L, a long position in two ATM ( at the money) put options with a strike price $X, and a short position in an ITM (in the money) put option with a strike price $X3H. The strikes are equidistant: $X3H minus $X equals to $X minus $X1L (Kakushadze & Serur, 2018).
#'@details
#'According to conceptual details given by Cohen (2015), and a closed form solution provided by Kakushadze and Serur (2018), this method is developed, and the given examples are created, to compute per share Profit and Loss at expiration for Short put butterfly Options Strategy and draw its graph in the Plots tab.
#'@param ST Spot Price at time T.
#'@param X1L Lower Strike Price or eXercise price for one OTM shorted put.
#'@param X Strike Price or eXercise price for two ATM bought puts.
#'@param X3H Higher Strike Price or eXercise price for one ITM shorted put.
#'@param P1L put Premium or put Price received for the first OTM shorted put.
#'@param P put Premium or put Price paid for the two ATM bought puts.
#'@param P3H put Premium or put Price received for the one ITM shorted put.
#'@param PnL Profit and Loss
#'@param spot Spot Price
#'@param pl Profit and Loss column of the data frame
#'@param myData Data frame
#'@param myTibble Table
...
#'@examples
#'shortPutButterfly(400,400,375,425,7,5,18,hl=0.9,hu=1.1)
#'@export
shortPutButterfly <- function (ST,X,X1L,X3H,P,P1L,P3H,hl=0,hu=2,spot=spot,pl=pl,myData=myData,myTibble=myTibble,PnL=PnL){

  V0 = (P1L + P3H - 2*P)
  myData <- data.frame (spot = c((ST*hl):(ST*hu)))
  myData$pl <- (2* (pmax((X-myData$spot),0))) - (pmax((X1L-myData$spot),0))- (pmax((X3H-myData$spot),0)) + V0
  ...
  ggplot(myTbColored,aes(x=spot,y=pl,fill=PnL,label=pl)) +
    geom_col(position = "identity",colour="deeppink1", size=0.5) +
    scale_fill_manual(values = c("darkviolet","cyan2"), guide= "none" ) +
    geom_point(aes(color=PnL), size=5)+
    scale_color_manual(values = c("deeppink1","green3"), guide= "none" ) +
    ...
    theme(panel.grid.major = element_line(colour  =  '#00B7B7',size  =  0.25))+
    theme(panel.grid.minor = element_line(colour  =  '#00B7B7',size  =  0.25))+
    theme(axis.title = element_text(colour  =  'lightcyan1', size=14))+
    theme(plot.title = element_text(colour  =  'snow1', vjust  =  1,size=16))+
    theme(panel.background = element_rect(fill  =  'turquoise4'))+
    theme(plot.background = element_rect(fill  =  '#00C1C1', colour  =  'aquamarine4', linetype  =  'dashed'))+
    labs(title  =  'Short Put Butterfly Options Strategy ', x  =  'Spot Price ($) at Expiration', y  =  'PnL ($) at Expiration', caption  =  'butterflyOptions / MaheshP Kumar')
}
