#'Calculates per share Profit and Loss (PnL) at expiration for Long Call Butterfly Option Strategy and draws its Bar Plot displaying PnL in the Plots tab.
#'@description
...
#'@param X1L Lower Strike Price or eXercise price for one ITM long Call.
#'@param X Strike Price or eXercise price for two ATM sold Calls.
#'@param X3H Higher Strike Price or eXercise price for one OTM long Call.
#'@param C1L Call Premium or Call Price paid for the first ITM long Call.
#'@param C Call Premium or Call Price received for the two ATM sold Calls.
#'@param C3H Call Premium or Call Price paid for the one OTM long Call.
...
#'@export
longCallbutterfly <- function (ST,X,X1L,X3H,C,C1L,C3H,hl=0,hu=2,spot=spot,pl=pl,myData=myData,myTibble=myTibble,PnL=PnL){
  V0= ((2*C)-C1L-C3H)
  myData <- data.frame (spot = c((ST*hl):(ST*hu)))
  myData$pl <- (pmax((myData$spot-X1L),0)) + (pmax((myData$spot-X3H),0))+ (2*(-pmax((myData$spot-X),0)))+V0
  ...
  ggplot(myTbColored,aes(x=spot,y=pl,fill=PnL,label=pl)) +
    ...
    labs(title  =  'Long Call Butterfly Strategy using Options', x  =  'Spot Price ($) at Expiration', y  =  'PnL ($) at Expiration', caption  =  'butterflyOptions / MaheshP Kumar')
}
